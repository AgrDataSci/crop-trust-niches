...
makeClimate <- function(path=".")
{
  
  climate <- stack(
    c(list.files(path, pattern="^tmin..bil$", full.names=TRUE),
      list.files(path, pattern="^tmin...bil$", full.names=TRUE),
      list.files(path, pattern="^tmean..bil$", full.names=TRUE),
      list.files(path, pattern="^tmean...bil$", full.names=TRUE),
      list.files(path, pattern="^prec..bil$", full.names=TRUE),
      list.files(path, pattern="^prec...bil$", full.names=TRUE)))
  return(climate)
...
makeClimate2 <- function(path=".")
{
  ...
  climate <- stack(
    c(list.files(path, pattern="^tmin_..asc$", full.names=TRUE),
      list.files(path, pattern="^tmin_...asc$", full.names=TRUE),
      list.files(path, pattern="^tmean_..asc$", full.names=TRUE),
      list.files(path, pattern="^tmean_...asc$", full.names=TRUE),
      list.files(path, pattern="^prec_..asc$", full.names=TRUE),
      list.files(path, pattern="^prec_...asc$", full.names=TRUE)))
  return(climate)
...
# Climate data
currentClimate <- makeClimate("Data/2000")

#Land area occupied by crops in 2000
cropland_5min <- raster("Data/Rasters/Cropland2000_5min.asc", native=TRUE)
extent(cropland_5min) <- extent(-180, 180,-90, 90) #There is a mistake in the extent
...
#Regions
regions <- readShapeSpatial("Data/Rasters/Regions_dissolved/Regions_dissolved_corrected")
regionnames <- as.vector(as.data.frame(regions)$Regions)
...
regions <- crop(regions_20min, currentClimate)
...
regions <- extract(regions, xyFromCell(cropland,cropCells), fun=function(x) modal(x, na.rm=T))
...
ld <- dir("/Data/Climate Data/RCP 4.5", pattern="rcp4_5_2050s", full.names=TRUE, include.dirs=TRUE)
ldout <- paste("/Results/Future", substr(ld, 16, 100), "/", sep="")
ldc <- c("/Results/Current", ldout)

# EcoCrop data
CropData <- read.csv("/Data/Datasets/EcocropParametersCropType6.csv")
